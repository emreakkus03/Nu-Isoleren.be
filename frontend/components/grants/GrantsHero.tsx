...
interface GrantsHeroProps {
  locale: string;
}

export default async function GrantsHero({
  locale,
}: GrantsHeroProps) {
  const t = await getTranslations({
    locale,
    namespace: 'GrantsPage.hero',
  });
    ...
    <section className="relative w-full overflow-hidden bg-[#123F5A]">
         ...
         <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 page-header-start pb-12 sm:pb-16 lg:pb-20">
           ...
           <div className="max-w-3xl flex flex-col text-center items-center md:items-start md:text-start gap-4">
             ...
             <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-white tracking-tight leading-[1.1] max-md:text-balance max-md:[overflow-wrap:anywhere]">
               {t('title')}
             </h1>
   
             <p className="text-sm sm:text-base md:text-lg text-white/80 leading-relaxed max-w-2xl max-md:text-center">
               {t('description')}
             </p>
           </div>
         </div>

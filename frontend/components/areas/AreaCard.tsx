    ...
    <div className="bg-slate-50 border border-slate-200 rounded-xl p-6 sm:p-7 flex flex-col justify-between shadow-sm hover:border-slate-300 transition-colors">
      <div>
        <div className="flex items-center gap-2.5 mb-5 border-b border-slate-200/80 pb-4">
          ...
          <h2 className="text-lg sm:text-xl font-extrabold text-slate-900 tracking-tight [overflow-wrap:anywhere]">
            {provinceName || 'Overige'}
          </h2>
        </div>

        <ul className="grid grid-cols-2 gap-x-3 gap-y-2 text-xs sm:text-sm text-slate-700">
  {cities.map((city) => (
    <li key={city.id} className="min-w-0">
      <Link
        href={{ pathname: '/areas/[slug]', params: { slug: city.slug } }}
        className="flex items-center gap-2 py-0.5 hover:text-[#C82024] transition-colors group"
      >
        <span className="w-1.5 h-1.5 rounded-full bg-[#1A669A] group-hover:bg-[#C82024] shrink-0" />
        <span className="truncate underline">{city.name}</span>
      </Link>
    </li>
  ))}
</ul>

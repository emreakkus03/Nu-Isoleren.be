...
export default async function AboutProjects() {
  const locale = await getLocale();
  const t = await getTranslations('AboutPage.projects');

  const projects = await getRecentProjects(locale, 3);
    ...
    <section className="w-full bg-white py-16 md:py-24">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 md:mb-14">
          <div className="flex flex-col gap-3 max-w-2xl max-md:text-center">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-950 tracking-tight leading-[1.15] max-md:text-balance max-md:[overflow-wrap:anywhere] max-md:w-full max-md:text-center">
              {t('title')}
            </h2>

            <p className="text-sm sm:text-base text-gray-600 leading-relaxed max-md:text-center">
              {t('description')}
            </p>
          </div>

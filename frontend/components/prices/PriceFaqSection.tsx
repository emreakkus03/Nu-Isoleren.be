...
export default async function PriceFaqSection() {
  const locale = await getLocale();
  const t = await getTranslations('PricesPage.faq');
          ...
          {faqs.map((faq) => (
            ...
            >
              <summary className="cursor-pointer list-none px-5 sm:px-8 py-4 sm:py-5 flex items-center justify-between gap-4 hover:bg-slate-50/70 transition">
                <span className="font-semibold text-slate-900 text-sm sm:text-lg leading-snug">
                  {faq.question}
                </span>

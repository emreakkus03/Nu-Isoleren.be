...
export interface EnergySavingsFaqItem {
  id: number | string;
  question: string;
  answer: string;
}

interface EnergySavingsFaqProps {
  faqs: EnergySavingsFaqItem[];
}

export default function EnergySavingsFaq({
  faqs,
}: EnergySavingsFaqProps) {
  const t = useTranslations(
    'EnergySavingsPage.faq'
  );
    ...
    <section className="w-full bg-white py-16 md:py-24">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          ...
          <p className="mt-5 max-w-2xl mx-auto text-base sm:text-lg text-gray-600 leading-relaxed max-md:text-center">
            {t('description')}
          </p>
        </div>
